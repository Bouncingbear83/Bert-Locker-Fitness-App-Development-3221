...
import { Dumbbell, TrendingUp, Calendar, Target, FileText, Plus, Zap, Award, Activity, CheckCircle, ArrowRight, Flame, Scale, TrendingDown } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const getWeightStats = () => {
    ...
    const recentEntries = userEntries.filter((entry) => new Date(entry.timestamp) >= weekAgo);
  ...
  const getTemplateCount = () => {
    ...
    return templates.filter((t) => t.userId === user?.id).length;
  };
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Scale,
      color: 'blue',
      trend: weightStats?.weeklyChange ? `${weightStats.weeklyChange > 0 ? '+' : ''}${weightStats.weeklyChange.toFixed(1)} ${weightStats.current.unit} this week` : 'No recent data',
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Link to="/exercises">
              <Button variant="secondary" className="w-full h-24 flex-col gap-2 p-4 group hover-lift text-slate-200">
                <Award className="h-6 w-6 group-hover:scale-110 transition-transform duration-200" />
            ...
            <Link to="/templates">
              <Button variant="secondary" className="w-full h-24 flex-col gap-2 p-4 group hover-lift text-slate-200">
                ...
                <div className="text-center">
                  <div className="font-semibold text-sm">Manage Templates</div>
